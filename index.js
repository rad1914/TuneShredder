...
const CFG = { sr: 22050, ch: 1, win: 4096, hop: 512, top: 24, zone: 55, pairs: 6, sec: 45, fq: 6, dtq: 2, bucket: 250 };
...
function topKIndices(row, K) {
  ...
  for (let i = 0; i < row.length; i++) {
    ...
    if (v > bestVal[minPos]) { bestVal[minPos] = v; bestIdx[minPos] = i; }
  }
  const r = [];
  for (let j = 0; j < K; j++) if (bestIdx[j] !== -1) r.push(bestIdx[j]);
  return r;
}
